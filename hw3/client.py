...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s: #Create a new socket using teh given address famiily, socket
    ...
    s.sendall(x.encode('utf-8')) #Send data to the socket. The socket must be connected to a remote socket. Data type
    #is byte
    ...
    print(f"Sent {x} and received {data!r}") #print to console